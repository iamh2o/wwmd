...
| WWMDApp | Menu-bar/native viewer with explicit signed-agent configuration, health, global-pause control, and user-requested bounded summary/recommendation views. | Sources/WWMDApp/main.swift. |
...
- User -> WWMDApp: user settings/annotation intent through a native UI. The
  ...
  then uses native XPC for health, global-pause control, and an explicit
  selected-range safe dashboard; it has no local database fallback. A
  release-configured signed service is still required.
...
| Menu-bar/viewer actions | User to app | Native UI to configured agent | The app validates only nonempty explicit service/signing inputs, tests native XPC off the main thread, enables pause only after authenticated health succeeds, and queries summary/recommendations only after the user requests a visible bounded date range; it does not fake a local persisted pause or open the database. | Sources/WWMDApp/main.swift. |
